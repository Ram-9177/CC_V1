...
export const onRequestGet: PagesFunction = async (ctx) => {
  ...
  const { id } = params;
  const result = await dataApi(env, 'find', { collection: 'students', filter: { _id: oid(id) }, limit: 1 });
...
export const onRequestPut: PagesFunction = async (ctx) => {
  ...
  const { id } = params;
  await dataApi(env, 'updateOne', { collection: 'students', filter: { _id: oid(id) }, update: { $set: body } });
...
export const onRequestDelete: PagesFunction = async (ctx) => {
  ...
  try { await verifyToken(env.JWT_SECRET, token); } catch { return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401 }); }
  const { id } = params;
  await dataApi(env, 'deleteOne', { collection: 'students', filter: { _id: oid(id) } });
  return new Response(JSON.stringify({ ok: true }), { headers: { 'Content-Type': 'application/json' } });
};

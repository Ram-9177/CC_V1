...
function Sidebar({ open, setOpen }: SidebarProps) {
  ...
  return (
    <>
      {open && (
        <div
          className="fixed inset-0 z-40 bg-gradient-to-b from-primary/15 via-background/70 to-background/85 backdrop-blur-sm transition-opacity dark:from-primary/10 dark:via-background/80 dark:to-background/90 lg:hidden"
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-[100] flex h-[100dvh] w-[280px] transform flex-col border-r border-primary/10 bg-gradient-to-b from-card/98 via-card/95 to-[hsl(var(--muted)_/_0.5)] shadow-2xl shadow-primary/10 backdrop-blur-xl transition-all duration-300 ease-out dark:border-border dark:from-card/98 dark:via-card/95 dark:to-[hsl(var(--muted)_/_0.55)] dark:shadow-black/35 lg:w-72 lg:translate-x-0",
          open ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div className="flex h-28 shrink-0 items-center justify-between border-b border-primary/10 bg-gradient-to-r from-primary/[0.04] to-transparent px-6 dark:border-border dark:from-transparent">
          ...
          <div className="space-y-10 pb-24">
            {filteredCategories.map((category) => (
              <div key={category.title} className="space-y-3">
                <h3 className="px-6 text-[11px] font-black uppercase tracking-widest text-primary/55 select-none dark:text-muted-foreground">
                  {category.title}
                </h3>
                  ...
                  {category.items.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          "group relative flex w-full items-center rounded-xl px-4 py-3 text-sm font-semibold transition-all duration-200",
                          isActive
                      ...
                      >
                        <item.icon className={cn(
                          ...
                          isActive ? "text-primary-foreground" : "text-primary/40 group-hover:text-primary dark:text-muted-foreground dark:group-hover:text-foreground"
                        )} />
                        <span className="relative z-10 flex-1 text-left min-w-0 truncate">{item.name}</span>
                        {item.count > 0 && (
                          <div className={cn(
                            "min-w-[20px] rounded-md px-2 py-0.5 text-center text-[10px] font-bold shadow-sm animate-in zoom-in duration-300",
                            isActive ? "bg-white/95 text-primary" : "bg-primary/10 text-primary dark:bg-muted dark:text-foreground"
                          )}>
                            {item.count}
                          </div>
                        )}
        ...
        <div className="p-6 border-t border-border space-y-3 bg-card shrink-0 pb-[calc(1.5rem+env(safe-area-inset-bottom,20px))]">
          ...
          <button
            ...
            className="group flex w-full items-center gap-3 rounded-sm px-4 py-3 text-left text-destructive transition-all hover:bg-destructive/10 active:scale-[0.98]"
          >

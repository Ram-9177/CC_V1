...
import { ResponsiveTabsNav } from '@/components/common/ResponsiveTabsNav';
  ...
  return (
    <div className="page-frame pb-14 sm:pb-16">
      ...
      <Tabs value={activeView} onValueChange={(value) => setActiveView(value as 'overview' | 'history')}>
        <ResponsiveTabsNav
          value={activeView}
          onValueChange={(v) => setActiveView(v as 'overview' | 'history')}
          options={[
            { value: 'overview', label: 'Overview' },
            { value: 'history', label: 'Pass history' },
          ]}
          selectLabel="Gate passes view"
        />
      </Tabs>
      ...
      <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-stretch">
        <div className="relative min-w-0 flex-1">
          ...
          <Input placeholder="Search hall ticket..." value={searchTicket} onChange={(e) => { setSearchTicket(e.target.value); setPage(1); }} className="h-10 rounded-lg border border-border bg-card pl-9 text-sm" />
        </div>
        <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v); setPage(1); }}>
          <SelectTrigger className="h-10 w-full shrink-0 rounded-lg border border-border bg-card text-xs font-medium sm:w-36">

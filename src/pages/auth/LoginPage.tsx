...
const getSoftLoginErrorMessage = (error: unknown): string => {
  ...
  if (status === 400 || status === 401 || status === 403 || code === 'API_ERROR' || code === 'INVALID_CREDENTIALS') {
    ...
    if (fromApi.trim()) {
      if (import.meta.env.DEV && /not\s+found/i.test(fromApi)) {
        return `${fromApi} If this is a fresh database, run \`python manage.py seed_test_users\` in the backend_django folder, then use hall ticket SUPERADMIN and password Test@1234.`
      }
      return fromApi
    }
...
export default function LoginPage() {
  ...
  const onSubmit = async (values: LoginFormValues) => {
    ...
    setIsLoading(true)
    try {
      // Match backend: case-insensitive hall ticket; strip accidental spaces (e.g. "SUPER ADMIN")
      const hallTicket = values.hall_ticket.trim().toUpperCase().replace(/\s+/g, '')
          ...
          <CardContent className="space-y-5 p-5 text-center sm:p-6">
            <div className="flex items-center justify-center">
              <img
                ...
                loading="lazy"
                className="h-14 w-auto max-w-[300px] object-contain opacity-80 grayscale"
              />
            </div>
            <div>
              <div className="mb-2 text-[10px] font-bold uppercase tracking-[0.2em] text-destructive">
              ...
              <h2 className="mb-1 text-xl font-black tracking-tight text-foreground">{collegeDisabled.collegeName}</h2>
              <div className="mx-auto mb-4 h-1 w-12 rounded-full bg-destructive/60" />
            ...
            <Button 
              variant="outline" 
              className="h-11 w-full rounded-xl border-2 border-border font-bold"
            ...
            <CardFooter className="flex flex-col space-y-3 px-5 pb-5 pt-2 sm:px-6 sm:pb-6">
              ...
              {import.meta.env.DEV && (
                <p className="text-center text-[11px] leading-relaxed text-muted-foreground">
                  Dev: run <code className="rounded bg-muted px-1 py-0.5 font-mono text-[10px]">python manage.py seed_test_users</code> in{' '}
                  <code className="rounded bg-muted px-1 py-0.5 font-mono text-[10px]">backend_django</code>, then sign in as{' '}
                  <span className="font-semibold text-foreground">SUPERADMIN</span> / <span className="font-semibold text-foreground">Test@1234</span>
                  (hall ticket is not case-sensitive).
                </p>
              )}
            </CardFooter>
          </form>

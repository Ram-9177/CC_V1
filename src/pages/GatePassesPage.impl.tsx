...
import { Plus, Search, QrCode, AlertCircle, Calendar as CalendarIcon, Clock, Play, MapPin, Info, CheckCircle2, ChevronDown, User as UserIcon, X } from 'lucide-react';
...
import {
  ...
  useVerifyGatePass,
  useScanQRCode,
} from '@/domains/gatePass/api';
import { useGatePassHistory } from '@/domains/gatePass/hooks/useGatePassHistory';
import { getStatusBadge, getMovementBadge, getTrackingPhase } from '@/domains/gatePass/utils/gatePassPresentation';
import { ProtocolModal } from '@/domains/gatePass/components/ProtocolModal';
...
export default function GatePassesPage() {
  ...
  const { history, setHistory, hasNextPage } = useGatePassHistory(queryData, page);
  ...
  return (
    <div className="page-frame pb-14 sm:pb-16">
                        ...
                        <Card className="rounded-xl border border-border bg-card shadow-sm active:scale-[0.99] transition-transform cursor-pointer overflow-hidden" onClick={() => {
                          ...
                          <CardHeader className="p-4 flex flex-row items-center justify-between">
                            <div className="flex items-center gap-3">
                              <div
                                className="h-10 w-10 bg-primary/10 rounded-sm flex items-center justify-center font-black text-primary hover:bg-primary/20 transition-colors"
                                onClick={(e) => {
                                  if (isAuthority || isSecurity) {
                                    e.stopPropagation();
                                    setSelectedStudentForCard(pass);
      ...
      <ProtocolModal
        pass={protocolPass}
        setProtocolPass={setProtocolPass}
        setSelectedStudentForCard={setSelectedStudentForCard}
